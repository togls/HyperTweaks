package io.github.togls.hypertweaks.core.config

object RemotePreferenceKeys {
    const val GroupName = "conf"

    const val ImeEnabled = "ime_enabled"
    const val NavBarLayoutStart = "nav_bar_layout_start"
    const val NavBarLayoutEnd = "nav_bar_layout_end"
    const val NavBarLayoutHandle = "nav_bar_layout_handle"

    const val GooglePhotosLocationEnabled = "google_photos_location_enabled"

    const val KeepAliveEnabled = "keep_alive_enabled"
    const val KeepAlivePackages = "keep_alive_packages"

    const val KeepAliveMode = "keep_alive_mode"
}
