...
class KeepAliveHook(
        ...
        runCatching {
            ...
            engine.hook(method) { chain ->
                    ...
                    if (
                        ...
                        return@hook defaultReturnValue(method.returnType)
                    }
    ...
    private fun observeSettings() {
        updateKeepAliveConfig(initialSettings)
        settingsSubscriptions += settingsProvider.subscribe { state ->
            updateKeepAliveConfig(state.snapshotOrDisabled())
        }
    }

    private fun updateKeepAliveConfig(settings: HookSettingsSnapshot) {
        updateKeepAlivePackages(settings.keepAlivePackages)
        updateKeepAliveMode(settings.keepAliveMode)
    }

    private fun updateKeepAliveMode(value: String) {
        val mode = KeepAliveMode.fromValue(value)
    ...
    private fun updateKeepAlivePackages(raw: String) {
